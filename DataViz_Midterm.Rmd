...
```{r}
df_filtered <- df %>%
  filter(Type1 %in% c("Ghost", "Rock") & !is.na(weight_kg) & !is.na(height_m)) %>%
  mutate(Type1 = factor(Type1, levels = c("Ghost", "Rock")))

ggplot(df_filtered, aes(x = weight_kg, y = height_m, color = Type1)) +
  ...
  labs(
    title = "Relationship Between Pokémon Weight and Height",
    subtitle = "Ghost and Rock Types",
    x = "Weight (kg)",
    y = "Height (m)",
    color = "Primary Type"
...
```{r}
df %>%
  filter(Type1 == "Rock" & !is.na(height_m)) %>%
...
ggplot(df_filtered, aes(x = weight_kg, y = height_m, color = Type1)) +
  ...
  labs(
    title = "Relationship Between Pokémon Weight and Height",
    subtitle = "Ghost and Rock Types (Onix Removed)",
    x = "Weight (kg)",
    y = "Height (m)",
    color = "Primary Type"
  ) +
